# Data Infrastructure & Engineering

## Core Engineering
| Category                | Data Engineer                                         | Data Architect                                     | Data Quality Engineer                          | Cloud Engineer                                  |
|-------------------------|------------------------------------------------------|--------------------------------------------------|-----------------------------------------------|------------------------------------------------|
| **Responsibilities**    | • Design & implement data pipelines<br>• Data integration  | • Define data strategy<br>• Design enterprise architecture | • Monitor data quality<br>• Define quality standards | • Design & implement cloud solutions<br>• Optimize costs |
| **Key Skills**          | • ETL/ELT<br>• Data warehousing<br>• Performance tuning       | • System integration<br>• Data governance<br>• Security      | • Data profiling<br>• Quality metrics<br>• Documentation | • Infrastructure as Code<br>• Security<br>• Performance    |
| **Languages**           | • Python<br>• SQL                                         | • SQL                                              | • SQL<br>• Python                                    | • Python<br>• YAML<br>• Shell scripting                   |
| **Tools**               | • Airflow<br>• dbt<br>• Spark                                | • Informatica<br>• Azure Data Factory<br>• Collibra         | • dbt<br>• Great Expectations<br>• Talend                | • Terraform<br>• Docker<br>• Kubernetes                   |
| **Platforms**           | • Snowflake<br>• AWS<br>• Azure                              | • AWS<br>• Azure<br>• GCP                                  | • Snowflake<br>• BigQuery<br>• Redshift                 | • AWS<br>• Azure<br>• GCP                                |


## Specialized Infrastructure

| Category                | Software Engineer (Data)                              | Data Platform Engineer                             | DevOps Engineer                                  |
|-------------------------|-----------------------------------------------------|--------------------------------------------------|------------------------------------------------|
| **Responsibilities**    | • Backend services<br>• Data APIs<br>• Microservices          | • Platform architecture<br>• Data systems<br>• Scalability  | • CI/CD pipelines<br>• Infrastructure<br>• Automation    |
| **Key Skills**          | • System design<br>• API development<br>• Testing             | • Distributed systems<br>• Data modeling<br>• Performance   | • DevOps practices<br>• Infrastructure<br>• Monitoring   |
| **Languages**           | • Java<br>• Python<br>• Go                                    | • Python<br>• Java<br>• SQL                                 | • Python<br>• Shell<br>• Go                              |
| **Tools**               | • Spring Boot<br>• FastAPI<br>• Docker                        | • Kubernetes<br>• Airflow<br>• Kafka                        | • Jenkins<br>• Terraform<br>• Docker                     |
| **Platforms**           | • AWS<br>• Azure<br>• GCP                                     | • Snowflake<br>• AWS<br>• Databricks                       | • AWS<br>• Azure<br>• GCP                                |

## Business Intelligence (BI) & Analytics

| Category                | BI Developer                                        | Data Analytics Engineer                           | Database Administrator                           |
|-------------------------|-----------------------------------------------------|--------------------------------------------------|------------------------------------------------|
| **Responsibilities**    | • Dashboard development<br>• Report creation<br>• Data modeling | • Analytics pipelines<br>• Data transformation<br>• Metrics | • Database management<br>• Performance tuning<br>• Security |
| **Key Skills**          | • Data visualization<br>• SQL modeling<br>• ETL              | • Analytics engineering<br>• Data modeling<br>• ETL/ELT    | • Database design<br>• Performance tuning<br>• Security    |
| **Languages**           | • SQL<br>• DAX<br>• Python                                   | • SQL<br>• Python<br>• dbt                                 | • SQL<br>• PL/SQL<br>• Shell scripting                    |
| **Tools**               | • Power BI<br>• Tableau<br>• Looker                          | • dbt<br>• Airflow<br>• Jupyter                            | • Oracle<br>• PostgreSQL<br>• MySQL                       |
| **Platforms**           | • Snowflake<br>• Azure Analysis Services<br>• BigQuery       | • Snowflake<br>• BigQuery<br>• Redshift                   | • Cloud databases<br>• On-prem databases<br>• Data lakes  |

# AI & Machine Learning

## Core AI & ML
| Category                | Data Scientist                                      | Machine Learning Engineer                          | MLOps Engineer                                   | AI Engineer                                      | ML Researcher                                    |
|-------------------------|-----------------------------------------------------|--------------------------------------------------|------------------------------------------------|------------------------------------------------|------------------------------------------------|
| **Responsibilities**    | • Statistical modeling<br>• Research<br>• Analysis          | • ML implementation<br>• Model deployment<br>• Optimization | • ML pipelines<br>• Model monitoring<br>• Automation   | • AI solutions<br>• Deep learning<br>• Architecture    | • Research methods<br>• Novel algorithms<br>• Publishing |
| **Key Skills**          | • Statistics<br>• ML algorithms<br>• Research               | • Software engineering<br>• ML systems<br>• Deployment    | • DevOps<br>• ML systems<br>• Monitoring              | • Deep learning<br>• System design<br>• Architecture   | • Mathematics<br>• Algorithm design<br>• Research      |
| **Languages**           | • Python<br>• R<br>• SQL                                    | • Python<br>• Java<br>• C++                              | • Python<br>• Go<br>• Shell scripting                | • Python<br>• C++<br>• CUDA                          | • Python<br>• R<br>• Julia                            |
| **Tools**               | • Jupyter<br>• scikit-learn<br>• TensorFlow                | • TensorFlow<br>• PyTorch<br>• MLflow                    | • Kubeflow<br>• MLflow<br>• Docker                   | • TensorFlow<br>• PyTorch<br>• CUDA                  | • Research tools<br>• ML frameworks<br>• HPC          |
| **Platforms**           | • AWS SageMaker<br>• Databricks<br>• Vertex AI             | • AWS<br>• Azure ML<br>• Vertex AI                       | • Kubernetes<br>• Cloud platforms<br>• MLflow         | • Cloud AI<br>• GPU clusters<br>• TPUs               | • HPC clusters<br>• Cloud research<br>• GPU farms     |

## Specialized ML

| Category                | NLP Engineer                                        | Computer Vision Engineer                          | Recommendation Systems Engineer                  |
|-------------------------|-----------------------------------------------------|--------------------------------------------------|------------------------------------------------|
| **Responsibilities**    | • Text processing<br>• Language models<br>• NLP pipelines     | • Image processing<br>• Vision models<br>• CV pipelines   | • RecSys design<br>• Model optimization<br>• A/B testing |
| **Key Skills**          | • NLP algorithms<br>• Text analytics<br>• Deep learning       | • Computer vision<br>• Image processing<br>• Deep learning | • RecSys algorithms<br>• Ranking models<br>• Evaluation  |
| **Languages**           | • Python<br>• Java<br>• R                                     | • Python<br>• C++<br>• CUDA                              | • Python<br>• Scala<br>• Java                           |
| **Tools**               | • NLTK<br>• spaCy<br>• Transformers                          | • OpenCV<br>• PyTorch<br>• TensorFlow                    | • Spark MLlib<br>• TensorFlow<br>• LightFM              |
| **Platforms**           | • AWS Comprehend<br>• Azure Language<br>• Cloud NLP          | • AWS Rekognition<br>• Azure Vision<br>• Cloud CV        | • AWS Personalize<br>• Azure Personalizer<br>• MLflow    |

# Analytics

## Core Analytics

| Category                | Data Analyst                                         | BI Analyst                                       | Analytics Consultant                           |
|-------------------------|-----------------------------------------------------|--------------------------------------------------|-----------------------------------------------|
| **Responsibilities**    | • Data analysis<br>• Reporting<br>• Insights generation      | • Dashboard creation<br>• KPI monitoring<br>• Reporting  | • Problem solving<br>• Client advisory<br>• Strategy   |
| **Key Skills**          | • Statistical analysis<br>• Data visualization<br>• SQL       | • Dashboard design<br>• Data modeling<br>• ETL          | • Business acumen<br>• Analytics<br>• Communication   |
| **Languages**           | • SQL<br>• Python<br>• R                                     | • SQL<br>• DAX<br>• Python                              | • SQL<br>• Python<br>• R                              |
| **Tools**               | • Excel<br>• Tableau<br>• Python libraries                   | • Power BI<br>• Tableau<br>• Looker                     | • Analytics tools<br>• BI platforms<br>• Excel        |
| **Platforms**           | • Snowflake<br>• BigQuery<br>• Redshift                     | • Snowflake<br>• Azure Analysis Services<br>• BigQuery  | • Cloud platforms<br>• Data warehouses<br>• BI tools  |


## Domain-Focused Analytics

| Category                | Marketing Analyst                                  | Financial Data Analyst                            | Product Analyst                                | Revenue Operations Analyst                      |
|-------------------------|--------------------------------------------------|--------------------------------------------------|-----------------------------------------------|------------------------------------------------|
| **Responsibilities**    | • Campaign analysis<br>• Attribution<br>• ROI tracking        | • Financial modeling<br>• Risk analysis<br>• Forecasting | • Product metrics<br>• User behavior<br>• A/B testing | • Revenue tracking<br>• Pipeline analysis<br>• Forecasting |
| **Key Skills**          | • Marketing analytics<br>• Attribution<br>• Segmentation     | • Financial analysis<br>• Risk modeling<br>• Accounting  | • Product analytics<br>• Experimentation<br>• UX       | • Sales analytics<br>• Revenue modeling<br>• Operations  |
| **Languages**           | • SQL<br>• Python<br>• R                                     | • SQL<br>• Python<br>• R                                 | • SQL<br>• Python<br>• R                              | • SQL<br>• Python<br>• R                                |
| **Tools**               | • Google Analytics<br>• Mixpanel<br>• Amplitude              | • Excel<br>• Tableau<br>• Bloomberg                      | • Amplitude<br>• Mixpanel<br>• Optimizely            | • Salesforce<br>• Tableau<br>• Excel                    |
| **Platforms**           | • GA4<br>• Adobe Analytics<br>• BigQuery                     | • Bloomberg<br>• FactSet<br>• Snowflake                  | • Snowflake<br>• BigQuery<br>• Amplitude              | • Salesforce<br>• Snowflake<br>• NetSuite               |


# Business & Leadership

## Data Governance & Compliance

| Category                | Data Steward                                        | Data Governance Specialist                         | Data Privacy Officer                            | Data Ethics Officer                             |
|-------------------------|-----------------------------------------------------|--------------------------------------------------|------------------------------------------------|------------------------------------------------|
| **Responsibilities**    | • Maintain data quality<br>• Define standards<br>• Documentation | • Implement governance<br>• Policy development<br>• Compliance | • Privacy compliance<br>• Data protection<br>• Risk management | • Ethical guidelines<br>• Impact assessment<br>• Fair practices |
| **Key Skills**          | • Data management<br>• Metadata<br>• Business knowledge        | • Policy framework<br>• Compliance<br>• Risk assessment      | • Privacy laws<br>• Security protocols<br>• Risk analysis    | • Ethics frameworks<br>• Impact analysis<br>• Policy design   |
| **Languages**           | • SQL<br>• Python<br>• R                                       | • SQL<br>• Python                                           | • SQL<br>• Python                                          | • Python<br>• R                                             |
| **Tools**               | • Collibra<br>• Alation<br>• Azure Purview                    | • Informatica<br>• Collibra<br>• OneTrust                   | • OneTrust<br>• BigID<br>• Privacera                      | • Ethics frameworks<br>• Assessment tools<br>• Documentation |
| **Platforms**           | • Snowflake<br>• Azure<br>• AWS                               | • Cloud platforms<br>• Data catalogs<br>• Compliance tools  | • Security platforms<br>• Cloud services<br>• DLP tools    | • Compliance platforms<br>• Assessment tools<br>• Analytics |

## Management & Strategy

| Category                | Manager                                            | Director                                         | Data Product Manager                            | Data Strategy Manager                           | Data Operations Manager                         |
|-------------------------|----------------------------------------------------|-------------------------------------------------|------------------------------------------------|------------------------------------------------|------------------------------------------------|
| **Responsibilities**    | • Team leadership<br>• Project execution<br>• Resource planning | • Department strategy<br>• Program oversight<br>• Growth planning | • Product roadmap<br>• Feature prioritization<br>• Stakeholder mgmt | • Strategy development<br>• Initiative planning<br>• Alignment | • Operations oversight<br>• Process optimization<br>• SLAs |
| **Key Skills**          | • People management<br>• Project mgmt<br>• Communication       | • Strategic planning<br>• Leadership<br>• Business acumen      | • Product management<br>• Technical knowledge<br>• Agile       | • Strategic thinking<br>• Business analysis<br>• Planning    | • Operations mgmt<br>• Process design<br>• Problem solving  |
| **Languages**           | • SQL<br>• Python<br>• R                                       | • SQL<br>• Python                                            | • SQL<br>• Python                                           | • SQL<br>• Python                                           | • SQL<br>• Python<br>• Shell scripting                     |
| **Tools**               | • JIRA<br>• Confluence<br>• BI tools                          | • Strategy tools<br>• Analytics<br>• Project mgmt            | • JIRA<br>• Product tools<br>• Analytics                    | • Strategy tools<br>• Analytics<br>• Planning tools         | • Monitoring tools<br>• JIRA<br>• Automation tools         |
| **Platforms**           | • Cloud platforms<br>• Data warehouses<br>• Analytics          | • Enterprise systems<br>• Cloud platforms<br>• Analytics      | • Data platforms<br>• Cloud services<br>• Analytics          | • Enterprise systems<br>• Analytics<br>• Planning tools     | • Cloud platforms<br>• Monitoring systems<br>• DevOps      |

## Leadership

| Category                | Head of Data                                       | VP of Data                                        | Chief Data Officer                              |
|-------------------------|---------------------------------------------------|--------------------------------------------------|------------------------------------------------|
| **Responsibilities**    | • Department leadership<br>• Strategy execution<br>• Team growth | • Enterprise strategy<br>• Executive leadership<br>• Innovation | • Data vision<br>• Digital transformation<br>• C-suite leadership |
| **Key Skills**          | • Leadership<br>• Strategy<br>• Technical expertise            | • Executive leadership<br>• Strategy<br>• Business acumen     | • Corporate strategy<br>• Executive presence<br>• Vision         |
| **Languages**           | • SQL<br>• Python<br>• R                                      | • SQL<br>• Python                                           | • SQL<br>• Python                                           |
| **Tools**               | • BI platforms<br>• Analytics suites<br>• Strategy tools      | • Enterprise platforms<br>• Analytics<br>• Executive tools    | • Enterprise systems<br>• Strategy tools<br>• Analytics         |
| **Platforms**           | • Cloud platforms<br>• Data warehouses<br>• Analytics         | • Enterprise systems<br>• Cloud platforms<br>• Analytics      | • Enterprise platforms<br>• Digital ecosystems<br>• Cloud        |

